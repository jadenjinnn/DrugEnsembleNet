...
class MultitoolAnalysis:
    ...
    def __init__(self, run_name, cell_type_name, cell_lines, dataset_path: Path, ppi_db, lincs_db, drugbank_db, ncbi_db):
        ...
        self.interactome = self.ppi.interactome
    ...
    def _get_promising_drug_candidates(self, results_df, drugbank):
        ...
        if results_df is None or results_df.empty:
            self.log.error("No significant results found for %s",
                           self.cell_type_name)
            return None

        proximal_results = results_df[
            results_df["DrugBank_ID"].isin(
                self.valid_drugbank_ids
            )
        ].copy()
        ...
        if self.network_proximity_results is None:
            self.log.error("Netwrok proximity empty for %s",
                           self.cell_type_name)
            return
        ...
        promising_drug_candidates[
            "Targets"
        ] = promising_drug_candidates["DrugBank_ID"].apply(
            lambda id: ", ".join(
                {
                    target.symbol
                    for target in drugbank.get(id).targets
                    if target.type == "protein" and target.organism == "Humans" and target.symbol is not None
                }
            )
        )
    ...
    def _get_genes_from_dataset_file(self, file_path):
        if not file_path.exists() or not file_path.is_file() or file_path.suffix != ".csv":
            self.log.error(
                "%s either does not exist, is not a file, or is not a csv file", file_path)
    ...
    def analyze(self):
        ...
        os.makedirs(
            f"data/results/{self.run_name}/{self.cell_type_name.replace(' ', '')}", exist_ok=True)
        ...
        with open(
            f"data/results/{self.run_name}/{self.cell_type_name.replace(' ', '')}/network_proximities.tsv",
            "r", encoding="utf-8"
        ) as infile:
            infile.readline()
            self.distance_threshold = float(
                re.findall(r"# Threshold: (-*[0-9]\.[0-9]{1,2})", infile.readline())[
                    0
                ]
            )
        ...
        self.log.info(
            "Successfully Performed Inverted Gene Set Enrichment Analysis")
        self.log.info("Performing Analysis with Single Set L2S2")

        self.l2s2_results = enrich_l2s2_single_set(
            list(self.disease_genes.keys()), self.drugbank, self.cell_type_name, self.run_name)

        self.log.info("Successfully Performed L2S2 Single Set Analysis")

        self.valid_drugbank_ids = set(
            self.network_proximity_results[
                self.network_proximity_results["Distance"]
                < self.distance_threshold
            ].index
        )

        self.log.info(
            "Selecting Promising Drug Candidates using IGSEA Results")
        ...
        if self.igsea_promising_drug_candidates is None:
            self.log.error(
                "No promising drug candidates found with IGSEA for %s", self.cell_type_name)
        else:
            self.igsea_promising_drug_candidates.to_csv(
                f"data/results/{self.run_name}/{self.cell_type_name.replace(' ', '')}/promising_drug_candidates_igsea.tsv",
                sep="\t",
            )

        self.log.info(
            "Selecting Promising Drug Candidates using L2S2 Results")
        ...
        if self.l2s2_promising_drug_candidates is None:
            self.log.error(
                "No promising drug candidates found with L2S2 for %s", self.cell_type_name)
        else:
            self.l2s2_promising_drug_candidates.to_csv(
                f"data/results/{self.run_name}/{self.cell_type_name.replace(' ', '')}/promising_drug_candidates_l2s2.tsv",
                sep="\t",
            )
...
def _run_worker(job_config):
    job_config["ppi_db"] = WORKER_DATABASES["ppi"]
    ...
    analyzer = MultitoolAnalysis(**job_config)
    analyzer.analyze()
...
class EnsemblePipeline:
    ...
    def run_parallel_analysis(self, max_workers=2):
        """
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers,
                                                    ...
                                                    initargs=(
                                                        self.all_jobs[0]["cell_lines"],)
                                                    ) as executor:
            results = list(tqdm(executor.map(_run_worker, self.all_jobs),
                                total=len(self.all_jobs), desc="Processing Cell Types"))
...
def gather_run_configs():
    ...
    first_time = input(
        "Do you want to download the required databases ? This only needs to be done if never done before. (y/n) ").lower()

    while first_time not in ["y", "n"]:
        first_time = input(
            "Response must be y or n ").lower()

    run_name = "_".join(input(
        "What is the name of the run? (This is typically the name of the dataset) ").split())
    cell_lines = input(
        "What cell lines do you want to use for analysis? Seperate each with a space ").split()
...
if __name__ == "__main__":
    ...
    if args.init:
        ...
        if not dir_path.exists() or not dir_path.is_dir() or not any(dir_path.iterdir()):
            log.error(
                "%s is not a directory, does not exist, or is empty", dir_path)

        os.makedirs(
            f"data/results/{run_config['run_name']}", exist_ok=True)
